...
Поиск в верхней панели находит разделы, материалы по содержимому и заметки. Кнопка «Продолжить» на главной открывает последнее посещённое место. Каждая заметка хранится отдельным TXT-файлом в пользовательской папке `Music Base/Data/Notes`; имена файлов совпадают с названиями заметок. Закрепление заметок сохраняется в `Music Base/Data/notes-pins.json`.

## Склад и локальные материалы

Материалы С склада публикуются в каталоге GitHub Releases. В админ-панели доступ проверяется по правам владельца репозитория; перед публикацией проверяются тип и размер обложки, а ZIP/RAR архивы тестируются 7-Zip. В карточке материала показываются дата публикации, жанры, размер файла и указанная администратором совместимость с FL Studio или Ableton Live.

При загрузке ZIP/RAR приложение проверяет архив, распаковывает его в Documents/Music Base/Library по категории и сохраняет список загрузок локально. Существующие файлы не перезаписываются. В настройках можно создать резервную копию TXT-заметок, закреплений, избранного и списка загрузок в Documents/Music Base/Backups.

Автоматического размещения файлов внутри папок FL Studio или Ableton Live нет. Поддерживаемые MIDI-файлы и пресеты по-прежнему можно перетащить из приложения в DAW.
